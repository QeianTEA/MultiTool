...
class PN532:
    ...
    def mifare_classic_write_block(self, block_number, data):
        """Write 16 bytes to the specified MIFARE Classic block (must authenticate first)."""
        assert len(data) == 16, "Data must be exactly 16 bytes"

        # Command format: [Card number, Write command (0xA0), block number] + 16 bytes data
        params = bytearray([0x01, MIFARE_CMD_WRITE, block_number]) + data
        ...
        if response is None or response[0] != 0x00:
            raise RuntimeError("Failed to write block")
